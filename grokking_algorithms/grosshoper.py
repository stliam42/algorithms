...
def count_trajectories(point, max_jump_lenght):
    ...
    for i in range(max_jump_lenght + 1, point + 1):
        for j in range(max_jump_lenght + 1):
            traj_matrix[i] += traj_matrix[i-j]

    return traj_matrix

print(count_trajectories(point, max_jump_lenght))